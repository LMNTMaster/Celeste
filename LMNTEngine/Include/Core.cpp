...
bool CCore::Init(HINSTANCE hInst, HWND hWnd, const RESOLUTION& tResolution, bool bWindowMode)
{
	...
	//SetClearColor(0xff, 0xC8, 0, 255); // LMNT_YELLOW
...
void CCore::Logic()
{
	...
	//if (fCollisionUpdateTimer >= fFixedTimeStep)
	//{
	//	fCollisionUpdateTimer -= fFixedTimeStep;
	//	if (GET_SINGLE(CCollisionManager)->UpdateCollision(fDeltaTime) == 1)
	//		return;
	//}
	...
	if (t >= 1)
	{
		...
		iCount = pCurrentScene->GetColliderList()->size();
	...
	if (GET_SINGLE(CSceneManager)->GetNextScene())
	{
	...
	GET_SINGLE(CInput)->ClearWheelState();
}
...
LRESULT CCore::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
	{
		...
		{
			GET_SINGLE(CInput)->SetWheelState(MOUSE_WHEEL_UP);
		}
		else
		{
			GET_SINGLE(CInput)->SetWheelState(MOUSE_WHEEL_DOWN);
		}
	}break;
	case WM_KEYDOWN:
	{
